...
export default function ContractsPage() {
  ...
  // =========================================================
  // FETCH CONTRACTS FROM NEXT.JS API
  // =========================================================
    ...
    const fetchContracts = async () => {
      try {
        ...
        if (!response.ok || !result.success) {
          throw new Error(
            result.error || "Failed to fetch contracts"
          );
        }
        ...
        setError("Unable to load contracts from Odoo.");
        setContracts([]);
      } finally {
        setLoading(false);
      }
  ...
  const getEmployeeName = (contract: Contract) => {
    if (
      Array.isArray(contract.employee_id) &&
      contract.employee_id.length > 1
    ) {
      return contract.employee_id[1];
    }
  ...
  const getEmployeeId = (contract: Contract) => {
    if (
      Array.isArray(contract.employee_id) &&
      contract.employee_id.length > 0
    ) {
      return String(contract.employee_id[0]);
    }
  ...
  const getSalaryStructure = (contract: Contract) => {
    if (
      Array.isArray(contract.salary_structure_id) &&
      contract.salary_structure_id.length > 1
    ) {
      return contract.salary_structure_id[1];
    }
  ...
  const runningContracts = contracts.filter(
    (contract) => contract.state.toLowerCase() === "running"
  ).length;

  const draftContracts = contracts.filter(
    (contract) => contract.state.toLowerCase() === "draft"
  ).length;

  const expiredContracts = contracts.filter(
    (contract) => contract.state.toLowerCase() === "expired"
  ).length;
  ...
  const filteredContracts = contracts.filter((contract) => {
    ...
    const employeeName = getEmployeeName(contract).toLowerCase();
    const employeeId = getEmployeeId(contract).toLowerCase();
    ...
    const matchesState =
      ...
      contract.state.toLowerCase() === stateFilter.toLowerCase();
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">
              Manage employee contracts, salary information,
              employment periods and contract status from one place.
            </p>
          </div>
          ...
          <table className="w-full min-w-[1100px]">
            ...
            <tbody>
              ...
              {filteredContracts.map((contract) => {
                ...
                const employeeId = getEmployeeId(contract);

                const salaryStructure =
                  getSalaryStructure(contract);
